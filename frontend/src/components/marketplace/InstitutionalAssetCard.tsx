...
export default function InstitutionalAssetCard({ asset, solPrice, index }: InstitutionalAssetCardProps) {
  const { formatPrice } = useCurrency();
  const soldPercentage = asset.soldPercentage || ((asset.totalSupply - asset.availableSupply) / asset.totalSupply * 100);
  
  // Mock Sparkline Data (simulate 7-day trend)
  const sparklineData = React.useMemo(() => [
    7.2, 7.3, 7.1, 7.4, 7.5, 7.4, 7.8
  ].map(v => v + (Math.random() * 0.5)), []);

  // Mock Risk Rating based on Asset Type
  const riskRatings: Record<string, string> = {
    residential: 'A-',
    commercial: 'A+',
    industrial: 'B+',
    land: 'B',
  ...
  const rating = riskRatings[asset.assetType] || 'A';
